...
const UI = {
  en: {
    ...
    features: 'Features', footerCatalog: 'Catalog metadata is kept current through automatic synchronization with published GitHub Releases',
    footerCatalogLink: 'Unity plugin catalog', footerDescription: 'Independent Unity Editor tools with predictable file changes, published source, and bilingual documentation.',
    footerAboutLink: 'About QuartzLab', footerSupportCopy: 'If QuartzLab saved time on a project, Boosty support helps keep releases free and maintenance focused.',
  ...
  ru: {
    ...
    features: 'Возможности', footerCatalog: 'Метаданные каталога автоматически синхронизируются с опубликованными релизами GitHub',
    footerCatalogLink: 'Каталог Unity-плагинов', footerDescription: 'Независимые Unity Editor-инструменты с предсказуемыми изменениями файлов, открытым кодом и документацией на двух языках.',
    footerAboutLink: 'О QuartzLab', footerSupportCopy: 'Если QuartzLab сэкономил время в проекте, поддержка на Boosty помогает оставлять плагины бесплатными и не раздувать их лишними системами.',
