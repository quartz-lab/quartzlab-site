...
- Синхронизация генерирует реальные clean URLs:
  - `public/en/plugins/{slug}/index.html`
  - `public/ru/plugins/{slug}/index.html`
  - `public/en/docs/{slug}/index.html`
  - `public/ru/docs/{slug}/index.html`
- Устаревшие сгенерированные docs и plugin routes для slug, которых больше нет в `catalog/plugins.config.json`, удаляются автоматически.
...
6. Копирует `Documentation~` на сайт целиком, включая HTML, CSS, JS и изображения.
...
3. Коммитит только сгенерированные изменения в:
   - `public/data`
   - `public/en/plugins`
   - `public/ru/plugins`
   - `public/en/docs`
...
- Все clean URLs обслуживаются реальными файлами `index.html`, без rewrite на `.html`.
